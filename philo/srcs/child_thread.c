...
/*   Updated: 2023/06/02 21:44:50 by seunghoy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_fork(t_philo *philo, t_status stat)
{
	...
	while (1)
	{
		...
		fork_stat = philo->info->fork[fork_index];
		...
		if (fork_stat == stat)
		{
			...
			pthread_mutex_unlock(philo->info->fork_mutex + fork_index);
			write_message(philo, GOT_FORK, get_time_us());
			return (0);
		}
		usleep(1);
	}
...
static int	philo_eat(t_philo *philo)
{
	...
	write_message(philo, EATING, now);
...
static int	philo_sleep(t_philo *philo)
{
	...
	if (!is_alive(philo))
		return (1);
	now = get_time_us();
	write_message(philo, SLEEPING, now);
